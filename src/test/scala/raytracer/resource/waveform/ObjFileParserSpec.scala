...
class ObjFileParserSpec extends BaseSpec {
  ...
  def parseObjFile(content: String): ObjResult = {
  ...
  feature("OBJ File Parser") {
    ...
    scenario("Vertex records") {
      ...
      assert(result.vertices(0) == point(-1, 1, 0))
      assert(result.vertices(1) == point(-1, 0.5, 0))
      assert(result.vertices(2) == point(1, 0, 0))
      assert(result.vertices(3) == point(1, 1, 0))
    }
    ...
    scenario("Parsing triangle faces") {
      ...
      assert(t1.p1 == result.vertices(0))
      assert(t1.p2 == result.vertices(1))
      assert(t1.p3 == result.vertices(2))
      assert(t2.p1 == result.vertices(0))
      assert(t2.p2 == result.vertices(2))
      assert(t2.p3 == result.vertices(3))
    }
    ...
    scenario("Triangulating polygons") {
      ...
      assert(t2.p2 == result.vertices(2))
      assert(t2.p3 == result.vertices(3))
      assert(t3.p1 == result.vertices(0))
      assert(t3.p2 == result.vertices(3))
      assert(t3.p3 == result.vertices(4))
    }
    ...
    scenario("Triangles in groups") {
      ...
      val g1 = group.collectChildren[Group].apply(0)
      val g2 = group.collectChildren[Group].apply(1)
      val t1 = g1.collectChildren[shapes.Triangle].head
      val t2 = g2.collectChildren[shapes.Triangle].head
      assert(t1.p1 == result.vertices(1 - 1))
      assert(t1.p2 == result.vertices(2 - 1))
      assert(t1.p3 == result.vertices(3 - 1))
      assert(t2.p1 == result.vertices(1 - 1))
      assert(t2.p2 == result.vertices(3 - 1))
      assert(t2.p3 == result.vertices(4 - 1))
    }
    ...
    scenario("Converting an OBJ file to a group") {
      ...
      val result = parseObjFile(trianglesFileContent)
      val g = result.toGroup
      val xs = g.collectChildren[Group]
      assert(xs.size == 2)
...
//      assert(g includes result.groups(1))
    ...
    scenario("Vertex normal records") {
      ...
      val result = parseObjFile(content)
      assert(result.normals(0) == vector(0, 0, 1))
      assert(result.normals(1) == vector(0.707, 0, -0.707))
      assert(result.normals(2) == vector(1, 2, 3))
    }
    ...
    scenario("Faces with normals") {
      ...
      assert(t1.p2 == result.vertices(1))
      assert(t1.p3 == result.vertices(2))
      assert(t1.n1 == result.normals(2))
      assert(t1.n2 == result.normals(0))
      assert(t1.n3 == result.normals(1))
      assert(t2 == t1)
    }
  }
}
