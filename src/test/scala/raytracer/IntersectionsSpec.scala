...
class IntersectionsSpec extends BaseSpec {
  ...
  feature("Intersections") {
    ...
    scenario("The under point is offset below the surface") {
      ...
      val r = ray(point(0, 0, -5), vector(0, 0, 1))
      val shape = glassSphere().setTransform(translation(0, 0, 1))
      val i = intersection(5, shape)
    ...
    forAll(indexes) { (index: Int, n1, n2) =>
      ...
      scenario(s"with index=$index, n1=$n1, n2=$n2") {
        ...
        val A = glassSphere()
          ...
          .updateMaterial(_.setRefractiveIndex(1.5))

        val B = glassSphere()
          .setTransform(translation(0, 0, -0.25))
          .updateMaterial(_.setRefractiveIndex(2.0))

        val C = glassSphere()
          .setTransform(translation(0, 0, 0.25))
          .updateMaterial(_.setRefractiveIndex(2.5))
