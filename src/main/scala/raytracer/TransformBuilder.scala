...
case class TransformBuilder private(operations: List[Operation]) {
  ...
  def identity: TransformBuilder =
    add(Identity)

  def translate(x: Double, y: Double, z: Double): TransformBuilder =
    add(Translation(x, y, z))

  def scale(n: Double): TransformBuilder = scale(n, n, n)
  def scale(x: Double, y: Double, z: Double): TransformBuilder =
    add(Scaling(x, y, z))

  def rotateX(r: Radians): TransformBuilder =
    add(RotationX(r))

  def rotateY(r: Radians): TransformBuilder =
    add(RotationY(r))

  def rotateZ(r: Radians): TransformBuilder =
    add(RotationZ(r))
